...
type PrevSnapshot = Record<FieldName, number> | null
...
export default function NewSnapshotForm({ prevSnapshot }: { prevSnapshot: PrevSnapshot }) {
  ...
  const [displayValues, setDisplayValues] = useState<Record<FieldName, string>>(emptyValues)
  ...
  function copyFromPrev() {
    ...
    setDisplayValues({
      cash_amount: prevSnapshot.cash_amount.toLocaleString('ja-JP'),
      investment_trust_amount: prevSnapshot.investment_trust_amount.toLocaleString('ja-JP'),
      stock_amount: prevSnapshot.stock_amount.toLocaleString('ja-JP'),
      buying_power_amount: prevSnapshot.buying_power_amount.toLocaleString('ja-JP'),
      other_amount: prevSnapshot.other_amount.toLocaleString('ja-JP'),
    })
  }
        ...
        <form action={formAction} className="flex flex-col gap-5">
          ...
          {FIELDS.map(({ name, label }) => (
            <div key={name} className="flex flex-col gap-1.5">
              ...
              <input type="hidden" name={name} value={displayValues[name]} />
              <input
                id={name}
                type="text"
                inputMode="numeric"
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              className="resize-none rounded-lg border border-black/[.08] bg-white px-3 py-2 text-sm text-foreground placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-black/20 dark:border-white/[.1] dark:bg-zinc-900 dark:placeholder:text-zinc-500 dark:focus:ring-white/20"
            />
          </div>
